...
Derived media lives alongside but apart from the footage: `qcp -proxy` builds a 720p browse tier and stills under a `proxies/` root, and `qcp -index` turns those into a static site you can browse with nothing plugged in. Proxies are never archived — they are regenerable, and cold space is the scarce resource.
...
the default: 720p H.264 at 2.5 Mbps with the log-to-Rec.709 transform baked in,
plus a poster frame and a 100-frame sprite sheet for hover-scrubbing. The whole
98-hour library is about 112GB at this tier, of which the stills are only
~750MB.
